...
public class MeteoParser {
	private String url = "http://www.ilmeteo.it/portale/meteo/previ.anas.php?";

	public String getMeteo(String comune, Date date) {
		url = "http://www.ilmeteo.it/portale/meteo/previ.anas.php?";
		if (comune.contains("(")) {
			comune = new StringTokenizer(comune, "(").nextToken().trim();
			if (comune.equals("Catanzaro Lido")) {
				comune = "Catanzaro";
			} else if (comune.equals("Gioiosa Jonica")) {
				comune = "Gioiosa%20Ionica";
			}
			else if(comune.equals("Le Castella di Isola Capo Rizzuto")){
				comune = "Isola%20di%20Capo%20Rizzuto";
			}
			else if(comune.equals("Tassitano di Aprigliano")){
				comune = "Aprigliano";
			}
			else{
		...
		try {
			...
			Elements tr = doc.select("tr[class=dark]");
			tr.addAll(doc.select("tr[class=light"));
			for (int i = 0; i < tr.size(); i++) {
				Element el = tr.get(i);
				Element a = el.select("a[class=active").first();
				if (a != null) {
